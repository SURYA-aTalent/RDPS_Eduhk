    ...
    */
    ...
    function Plugin ( element, options ) {
        ...
        */
        ...
        this._defaults = $.fn.nameSearch.defaults;
        /*
    ...
    $.extend(Plugin.prototype, {
        ...
        checking: function() {
        	if (!jQuery().select2) {
        		throw "Query Select2 Library not found";
        	}
        },
        initSelect2: function() {
            var plugin = this;
            
            var $select2Ele = plugin.$element;
            /** if the element is <input>, create a select and hide the input **/
            if (plugin.$element.is("input")) {
            	plugin.$element.hide();
            	$select2Ele = $("<select>").prop("name", plugin.$element.prop("name"))
            		.prop("id", plugin.$element.prop("id"))
            		.prop("class", plugin.$element.prop("class"));
            	plugin.$element.after($select2Ele);
            	plugin.$element.prop("name", plugin.$element.prop("name") + "_hidden");
            	plugin.$element.prop("id", plugin.$element.prop("id") + "_hidden");
            }
            ...
            var format = function(state) {
            	...
            	return $("<div>" + state.displayName + " <small>("+ state.instituteId +")</small><br>" + state.department + " - " + state.rankDesc + "</div>");
            }
            ...
            $select2Ele.select2({
            	ajax: {
            		...
            		processResults: function(res) {
            			var select2Data = $.map(res.result, function(obj) {
            				obj.id = obj.instituteId;
            				obj.text = obj.instituteId;
            				return obj;
            			});
            	...
            	placeholder: {id: -1, text: "Type staff number, staff name or department to search"},
            	minimumInputLength: 2,
//            	theme: "bootstrap4",
            	dropdownParent: plugin.$element.closest("div.bootbox-body"),
            	...
            	templateSelection: function(d) {
            		if (d.instituteId) {
            			return d.instituteId + " - " + d.displayName;
            		} else {
            			return d.text;
            		}
            	}
...
//            alert('I promise to do something cool!');
    ...
    */
    $.fn.nameSearch = function ( options ) {
        this.each(function() {
